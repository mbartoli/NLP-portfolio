...
/**
 * A language model using the Interpolation model
 * 
 * @author Aloke Desai and Mike Bartoli
 * @version 9/23/2014
 *
 */
public class InterpolationLMModel implements LMModel {
	...
	public double getBigramProb(String first, String second) {
		...
		if (bigramProbability.get(first).containsKey(second)) {
			initProbability = bigramProbability.get(first).get(second);
		}

		// calculate probability using interpolation
		double probability = lambda * initProbability + (1-lambda) * (unigramFrequency.get(second)/wordCount);
		return probability;
	}
	...
	public static void main(String[] args) {
		...
		for (double lambda: lambdas) {
			...
			System.out.println(lambda + ": " + lm.getPerplexity("development.txt"));
		}
	}
}
